...
export async function sendOtpEmail({ toEmail, otp, userName = "Music Lover" }) {
  ...
  // 1. Primary Cloud Provider: Resend (HTTPS Port 443 — Never blocked by Render, Vercel, or AWS)
  const resend = getResendClient();
  if (resend) {
    try {
      const fromAddress =
        process.env.RESEND_FROM?.trim() || "Pulse Music <onboarding@resend.dev>";
      ...
      if (error) {
        throw new Error(error.message || "Resend API delivery failed");
      }

      console.log(
        `[PULSE RESEND API] Live OTP email delivered via HTTPS to ${toEmail}. ID: ${data?.id}`
      );
      ...
      console.warn(
        `[PULSE RESEND API] Resend failed (${resendError.message}), attempting SMTP fallback...`
      );
    }
  }

  // 2. Secondary Provider: Nodemailer SMTP
  ...
  if (transporter) {
    try {
      ...
      console.log(
        `[PULSE SMTP SERVER] Live OTP email sent successfully to ${toEmail}. Message ID: ${info.messageId}`
      );
      ...
      console.error("[PULSE SMTP SERVER] SMTP delivery failed:", smtpError.message);
      throw smtpError;
    }
  }

  // 3. Fallback for Local Development (Console Log)
  console.log("\n=======================================================");
  ...
  console.log(
    `⚠️ Configure RESEND_API_KEY or SMTP credentials in .env for live delivery.`
  );
  console.log("=======================================================\n");
